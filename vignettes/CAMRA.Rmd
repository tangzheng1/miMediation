...
knitr::include_graphics("CAMRAexample.png")
```
...
$$\mathbb{E}[Y_{i}|E_{i},\log(W_{i})]=\beta_{0}+E_{i}\beta_{E}+\sum_{k=1}^{p}\log(W_{ik})\beta_{k}$$
...
CAMRA provides two implementations for composite-null mediation testing step. When `method = "hdmt"`, CAMRA applies HDMT to obtain taxon-level q-values by explicitly accounting for the mixture structure of the composite null. When `method = "sbmh"`,
CAMRA uses the Sampson-Boca-Moore-Heller procedure to construct subset-adjusted mediation p-values, followed by FDR adjustment.
...
data("data.bmi")
E <- data.bmi$exposure
table(E) # 0: China, 1: United States
